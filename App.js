...
export default function Home() {
  ...
  const adicionarProdutoExemplo = async () => {
    if (!db) return;

    try {
      const exemplos = [
        { nome: 'Smartphone', preco: 999.99, descricao: 'Celular de última geração' },
        { nome: 'Notebook', preco: 2500.0, descricao: 'Notebook para trabalho' },
        { nome: 'Fone de Ouvido', preco: 199.99, descricao: 'Fone Bluetooth' },
      ];
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>{produtos.length} itens</Text>
      </View>
      ...
      <View style={styles.content}>
        {produtos.length === 0 ? (
          <View style={styles.empty}>
            ...
            <TouchableOpacity
              style={styles.addBtn}
              onPress={adicionarProdutoExemplo}
            >
      ...
      <TouchableOpacity style={styles.fab} onPress={adicionarProdutoExemplo}>
...
const styles = StyleSheet.create({
  ...
  plus: { color: '#fff', fontSize: 25, fontWeight: 'bold' },
});
